...
class Business extends Controller
{
    public function index()
    {
      ...
      // exit;
        ...
        if(isset($data['searchname'])){
            ...
            $house = SecondHandHouseModel::order('update_time','desc')->where('title','like','%'.$searchname.'%')->where('type',SecondHandHouseModel::$types['business']['val'])->where("checkout",1)->paginate(9,false,['query'=>['searchname'=>$searchname]]);
        }else{
            $house = SecondHandHouseModel::where(function($query)use($data){
                ...
                if(isset($data['price']) && $data['price']!='0'){
                    ...
                    if($max_price==0){
                        $query->where('price1','>=',$min_price*10000);
                    }else{
                        $query->where('price1','>=',$min_price*10000)->where('price1','<=',$max_price*10000);
                    }
                }
                ...
                if($data['price_min']!=""){
                    $query->where('price1','>=',$data['price_min']*10000);
                }
                if($data['price_max']!=""){
                    $query->where('price1','<=',$data['price_max']*10000);
                }
                if($data['house_min']!=""){
        ...
        $hothouse=SecondHandHouseModel::order('is_hot','desc')->where('type',SecondHandHouseModel::$types['business']['val'])->where("checkout",1)->order('read_count','desc')->limit(0,20)->select();
    ...
    public function sendinfo()
    {
        if($_POST){
            ...
            if(!empty($files)){
              ...
              $this->success("提交成功，等待后台审核",'Business/index');
        }
        $hothouse=SecondHandHouseModel::where('type',SecondHandHouseModel::$types['business']['val'])->order('read_count','desc')->select();
        ...
        $buy_up=AdModel::where('position','business_up')->find();
        $buy_down=AdModel::where('position','business_down')->find();
        $this->assign('hothouse',$hothouse);
        $this->assign('area',$area);
        $this->assign('price',$price);
        $this->assign('layout',$layout);
    ...
    public function info_detaile()
    {
        $id=$_GET['id'];
        $house=SecondHandHouseModel::where('type',SecondHandHouseModel::$types['business']['val'])->find($id);
        if(!$house){
             $this->error('不要调皮=_=');
        }
        ...
        $sale_up=AdModel::where('position','business_up')->find();
        $sale_down=AdModel::where('position','business_down')->find();
