...
class Hotroombed extends Controller
{
    public function repanxianfang()
    {
        Session::set('title','xianfang');
        $data=request()->get();
        ...
        if(isset($data["searchname"])){
            ...
            $build = BuildingModel::where('name','like','%'.$searchname.'%')->order('update_time','desc')->where('is_delete',BuildingModel::$isDelete['no']['val'])->where('state',BuildingModel::$state['xianfang']['val'])->paginate(9,false,['query'=>['searchname'=>$searchname]]);
        }else{
        ...
        $hotbuild=BuildingModel::where('is_delete',BuildingModel::$isDelete['no']['val'])->where('state',BuildingModel::$state['xianfang']['val'])->order('is_hot','desc')->order('read_count','desc')->limit(0,8)->select();
        ...
        $xianfang_up=AdModel::where('position','xianfang_up')->find();
        $xianfang_down=AdModel::where('position','xianfang_down')->find();

        $this->assign('xianfang_up',$xianfang_up);
        $this->assign('xianfang_down',$xianfang_down);
